...
uint16_t dbuf_read_u16(dbuf* const buffer)
{
    ...
    uint16_t value = *(uint16_t*)dbuf_pos(buffer);
    dbuf_advance(buffer, sizeof(uint16_t));
...
uint32_t dbuf_read_u32(dbuf* const buffer)
{
    ...
    dbuf_advance(buffer, sizeof(uint32_t));
...
uint8_t* dbuf_read_u8_array(dbuf* const buffer, size_t count)
{
    ...
    memcpy(result, dbuf_pos(buffer), count);

    dbuf_advance(buffer, count);
...
uint16_t* dbuf_read_u16_array(dbuf* const buffer, size_t count)
{
    ...
    memcpy(result, dbuf_pos(buffer), count * sizeof(uint16_t));
    dbuf_advance(buffer, count * sizeof(uint16_t));
...
uint32_t* dbuf_read_u32_array(dbuf* const buffer, size_t count)
{
    ...
    memcpy(result, dbuf_pos(buffer), count * sizeof(uint32_t));
    dbuf_advance(buffer, count * sizeof(uint32_t));
...
void dbuf_write_u16(dbuf* const buffer, uint16_t value)
{
    ...
    dbuf_write_data(buffer, &value, sizeof(uint16_t), true);
}

void dbuf_write_u32(dbuf* const buffer, uint32_t value)
{
    value = endian_swap_u32(value);
    dbuf_write_data(buffer, &value, sizeof(uint32_t), true);
}
...
{
    value = endian_swap_uvar(value, stride);
    dbuf_write_data(buffer, &value, stride, true);
}


void dbuf_write_u8_array(dbuf* const buffer, uint8_t* const src, size_t count)
{
    dbuf_write_data(buffer, src, count, true);
}
...
void dbuf_write_u16_array(dbuf* const buffer, uint16_t* const src, size_t count)
{
    ...
    if(!endian_should_convert())
    {
        dbuf_write_data(buffer, src, count * sizeof(uint16_t), true);
    }
    else
    {
        for(size_t i = 0; i < count; i++)
        {
            dbuf_write_u16(buffer, src[i]);
        }
    }
}
...
void dbuf_write_u32_array(dbuf* const buffer, uint32_t* const src, size_t count)
{
    ...
    if(!endian_should_convert())
    {
        dbuf_write_data(buffer, src, count * sizeof(uint32_t), true);
    }
    else
    {
        for(size_t i = 0; i < count; i++)
        {
            dbuf_write_u32(buffer, src[i]);
        }
    }
}
...
void dbuf_write_uvar_array(dbuf* const buffer, uint32_t* const src, size_t count, unsigned int stride)
{
    ...
    if(!endian_should_convert())
    {
        dbuf_write_data(buffer, src, count * stride, true);
    }
    else
    {
        for(size_t i = 0; i < count; i++)
        {
            dbuf_write_uvar(buffer, src[i], stride);
        }
    }
}
